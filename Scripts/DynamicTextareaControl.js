...
(function Controls(ns) {
	ns.DynamicTextareaEl = class DynamicTextareaEl extends HTMLElement {
		...
		constructor() {
			...
			if(this.InstanceId === 0) {
				ns.DynamicTextareaEl.EditorMode = localStorage.getItem("gw-dynamic-textarea-editor-mode") === "on";
				...
				ns.DynamicTextareaEl.AsiPolite = document.createElement("aside");
				ns.DynamicTextareaEl.AsiPolite.setAttribute("aria-live", "polite");
				ns.DynamicTextareaEl.AsiPolite.setAttribute(
					"style",
					"position: absolute; left: -99999999px; top: 0px;"
				);
				document.body.insertAdjacentElement("beforeend", ns.DynamicTextareaEl.AsiPolite);
			}
		}

		getId(key) {
			return `gw-dynamic-textarea-${this.InstanceId}-${key}`;
		}
		getRef(key) {
		...
		onToggleClick = (event) => {
			...
			ns.DynamicTextareaEl.AsiPolite.insertAdjacentHTML("afterbegin", `
				<article id="${this.getId("msg-" + ++this.MessageIdx)}">
					Tab now ${ns.DynamicTextareaEl.EditorMode  ? "adjusts indentation" : "moves focus"}
				</article>
			`);
			setTimeout((idx) => {
				document.getElementById(this.getId("msg-" + idx)).remove();
			}, 100, this.MessageIdx);

			Array.from(document.querySelectorAll("gw-dynamic-textarea")).forEach(element => element.updateState());
		};

		updateState() {
			if(ns.DynamicTextareaEl.EditorMode) {
	...
	customElements.define("gw-dynamic-textarea", ns.DynamicTextareaEl);
